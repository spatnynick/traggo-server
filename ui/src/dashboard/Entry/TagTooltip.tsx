...
export const TagTooltip = ({
    ...
    total,
    formatDuration,
}: TooltipProps & {dateFormat: FInterval; total: boolean; formatDuration: (totalSeconds: number) => string}) => {
    if (active && payload) {
            ...
            <Paper style={{padding: 10}} elevation={4}>
                ...
                {payload.map((entry) => (
                    <Typography key={entry.name}>
                        {entry.name}: {formatDuration(entry.payload.data[entry.name] as number)}
                    </Typography>
                ))}
                {total ? (
                    <Typography>
                        Total:
                        {formatDuration(sum(payload))}
                    </Typography>
                ) : (
                    undefined
                )}
